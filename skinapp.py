...
# Cargar los datos
data = pd.read_csv('basedatos.csv')  # Asegúrate de que el archivo esté en la misma carpeta que este script

# Limpieza de nombres de columnas
data.columns = data.columns.str.strip()  # Elimina espacios extra al inicio o final
data.columns = data.columns.str.lower()  # Convierte los nombres a minúsculas para evitar errores
...
rango_precio = st.sidebar.slider("Seleccionar rango de precio", min_value=0, max_value=int(data["precio"].str.extract(r'(\d+)', expand=False).astype(float).max()), value=(0, 1000))
...
productos_filtrados = productos_filtrados[
    (productos_filtrados["precio_num"] >= rango_precio[0]) & (productos_filtrados["precio_num"] <= rango_precio[1])
]
